...
import com.example.productcatalog.data.mapper.toProductDetail
...
class ProductRepositoryImpl @Inject constructor(
    ...
    override suspend fun getProducts(page: Int): Async<PaginatedProducts> = withContext(ioDispatcher) {
        runCatching {
    ...
    override suspend fun getProductDetail(productId: Int): Async<ProductDetail> = withContext(ioDispatcher) {
        runCatching {
            apiService.getProductById(productId)
        }.fold(
            onSuccess = { response ->
                Async.Success(data = response.toProductDetail())
            },
            onFailure = { error ->
                Async.Error(
                    errorMessage = error.toErrorMessage(),
